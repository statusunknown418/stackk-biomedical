...
> {
  ...
  min(
    value: number,
  ): ColumnConfigBuilder<TData, TType extends "number" ? TType : any, TVal, TId> {
  ...
  options(
    ...
    TData,
    TType extends "option" | "multiOption" ? TType : any,
    TVal,
    TId
  > {
...
export function createColumns<TData>(
  ...
  return columnConfigs.map((columnConfig) => {
    ...
    // @ts-expect-error - TS doesn't like the memo() calls
