...
export interface ColumnConfig<
  TData,
  TType extends ColumnDataType = never,
  TVal = unknown,
...
export type OptionColumnId<T> =
  T extends ColumnConfig<unknown, "option" | "multiOption", unknown, infer TId>
    ? TId
    : never;

export type OptionColumnIds<
  T extends readonly ColumnConfig<unknown, never, unknown, never>[],
> = {
  [K in keyof T]: OptionColumnId<T[K]>;
}[number];

export type NumberColumnId<T> =
  T extends ColumnConfig<unknown, "number", unknown, infer TId> ? TId : never;
...
export interface ColumnProperties<TVal> {
...
export interface ColumnPrivateProperties<TVal> {
 ...
 *
 */
export interface FilterModel<TType extends ColumnDataType = never> {
  columnId: string;
  type: TType;
...
export interface FilterOperatorDetailsBase<OperatorValue, T extends ColumnDataType> {
