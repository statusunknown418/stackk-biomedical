  ...
  if (selected.length === 1) {
    ...
    return (
      <span className="inline-flex items-center gap-1">
        {hasIcon &&
          (isValidElement(Icon) ? Icon : <Icon className="text-primary size-4" />)}
        <span>{label}</span>
      </span>
    );
  }
...
export function FilterValueDateDisplay<TData>({
  ...
  const formattedRangeStr = formatDateRange(
    filter.values[0] ?? new Date(),
    filter.values[1] ?? new Date(),
  );
...
export function FilterValueNumberDisplay<TData>({
  ...
  if (!filter.values.length || filter.values.length === 0) return null;
...
export function FilterValueNumberController<TData>({
  ...
  const isNumberRange =
    ...
    filter.values.length && numberFilterOperators[filter.operator].target === "multiple";
  ...
  const changeNumber = (value: number[]) => {
    setValues(value);
    setFilterValueDebounced(column as never, value);
  };

  const changeMinNumber = (value: number) => {
    const newValues = createNumberRange([value, values[1] ?? 0]);
    setValues(newValues);
    setFilterValueDebounced(column as never, newValues);
  };
