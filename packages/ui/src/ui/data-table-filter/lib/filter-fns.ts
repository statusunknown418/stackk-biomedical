...
export function dateFilterFn(inputData: Date, filterValue: FilterModel<"date">) {
  if (!filterValue.values.length || filterValue.values.length === 0) return true;
  ...
  switch (filterValue.operator) {
    case "is":
      return isSameDay(value, d1 ?? new Date());
    case "is not":
      return !isSameDay(value, d1 ?? new Date());
    case "is before":
      return isBefore(value, startOfDay(d1 ?? new Date()));
    case "is on or after":
      return (
        isSameDay(value, d1 ?? new Date()) || isAfter(value, startOfDay(d1 ?? new Date()))
      );
    case "is after":
      return isAfter(value, startOfDay(d1 ?? new Date()));
    case "is on or before":
      return (
        isSameDay(value, d1 ?? new Date()) ||
        isBefore(value, startOfDay(d1 ?? new Date()))
      );
    case "is between":
      return isWithinInterval(value, {
        start: startOfDay(d1 ?? new Date()),
        end: endOfDay(d2 ?? new Date()),
      });
    case "is not between":
      return !isWithinInterval(value, {
        start: startOfDay(filterValue.values[0] ?? new Date()),
        end: endOfDay(filterValue.values[1] ?? new Date()),
      });
  }
}
...
export function numberFilterFn(inputData: number, filterValue: FilterModel<"number">) {
  if (!filterValue.values.length || filterValue.values.length === 0) {
    return true;
  }
